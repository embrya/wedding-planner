...
create index calendar_sync_queue_due_idx
  on public.calendar_sync_queue (next_attempt_at, planner_id)
  where locked_at is null;
create index calendar_event_links_wedding_idx
...
create function public.upsert_calendar_sync_job(
...
create function public.queue_day_note_calendar_sync()
returns trigger
language plpgsql security definer set search_path = ''
as $$
declare
...
create function public.queue_wedding_calendar_sync()
returns trigger
language plpgsql security definer set search_path = ''
as $$
declare
...
begin
  if tg_op = 'DELETE' then
    ...
    loop
      perform public.upsert_calendar_sync_job(
        old.planner_id, link_record.source_type, link_record.wedding_id,
        link_record.source_key, 'delete', '{}'::jsonb
      );
    end loop;
    return old;
  end if;
  ...
  if metadata_changed or reactivated then
    ...
    for note_record in select note_date from public.day_notes where wedding_id = new.id loop
      perform public.upsert_calendar_sync_job(
        new.planner_id, 'day_note', new.id, note_record.note_date::text,
        'upsert', jsonb_build_object('note_date', note_record.note_date)
      );
    end loop;
  end if;
  return new;
